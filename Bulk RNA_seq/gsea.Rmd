...
GSEAgo <- function(filedir,condition,tissue,outputdir){
  ...
  inputdir <- str_c("/data/msun/projects/02_DEG/edgeR/",filedir,"/",tissue,"/",tissue,file)
  ...
  go <- gseGO(geneList     = geneList,
              ...
              verbose      = FALSE,
              nPermSimple = 10000)
  outputad <- str_c("/data/msun/projects/06_GSEA/edgeR/go/",outputdir,"/",tissue,"/",tissue,file)
...
GSEAKEGG <- function(filedir,condition,tissue,outputdir){
  ...
  KEGG <- gseKEGG(geneList = geneList,
              ...
              verbose      = FALSE,
              nPermSimple = 10000)
  outputad <- str_c("/data/msun/projects/06_GSEA/edgeR/kegg/",outputdir,"/",tissue,"/",tissue,file)
...
GSEAgo <- function(filedir,condition,tissue,outputdir){
  file <- paste(condition[1],condition[2],sep = "vs")
  inputdir <- str_c("/data/msun/projects/02_DEG/DESeq2/",filedir,"/",tissue,"/",tissue,file)
  df <- read.csv(paste(inputdir,"csv",sep = ".")) 
  ...
  go <- gseGO(geneList     = geneList,
              ...
              verbose      = FALSE,
              nPermSimple = 10000)
  outputad <- str_c("/data/msun/projects/06_GSEA/DESeq2/go/",outputdir,"/",tissue,"/",tissue,file)
  write.csv(go,file = str_c(outputad,".csv"))
  print(str_c(tissue,condition))
}
pwalk(walktable, ~ GSEAgo(..1,..2,..3,..4))
...
GSEAKEGG <- function(filedir,condition,tissue,outputdir){
  ...
  KEGG <- gseKEGG(geneList = geneList,
              ...
              verbose      = FALSE,
              nPermSimple = 10000)
  outputad <- str_c("/data/msun/projects/06_GSEA/DESeq2/kegg/",outputdir,"/",tissue,"/",tissue,file)
  write.csv(KEGG,file = str_c(outputad,".csv"))
}
pwalk(walktable, ~ GSEAKEGG(..1,..2,..3,..4))
```
...
combinealldata <- function(tissue){
  edger.sedrw <- list.files(path = paste("/data/msun/projects/06_GSEA/edgeR/go/table_sedvsrw",tissue,sep = "/"),full.names = TRUE) %>% 
    map(myfuns) %>% purrr::reduce(inner_join, by = "ID")
  
  edger.zt <- list.files(path = paste("/data/msun/projects/06_GSEA/edgeR/go/table_zt4vs16",tissue,sep = "/"), full.names = TRUE) %>% 
    map(myfuns) %>% purrr::reduce(inner_join, by = "ID")
  
  deseq.sedrw <- list.files(path = paste("/data/msun/projects/06_GSEA/DESeq2/go/table_sedvsrw",tissue,sep = "/"), full.names = TRUE) %>% 
    map(funs) %>% purrr::reduce(inner_join, by = "ID")
  
  deseq.zt <- list.files(path = paste("/data/msun/projects/06_GSEA/DESeq2/go/table_zt4vs16",tissue,sep = "/"), full.names = TRUE) %>% 
    map(funs) %>% purrr::reduce(inner_join, by = "ID")

    #go info
  go <- read.csv("/data/msun/projects/06_GSEA/DESeq2/go/table_sedvsrw/BF/BFF_16_SedvsF_16_RW.csv",row.names = 1) %>% dplyr::select(1:4)
  #last merge
  ...
  openxlsx::write.xlsx(combined.table,file =
              paste(paste("/data/msun/projects/06_GSEA/combined_go",tissue,sep = "/"),".xlsx",sep = ""))
}
...
combinealldata <- function(tissue){
  edger.sedrw <- list.files(path = paste("/data/msun/projects/06_GSEA/edgeR/kegg/table_sedvsrw",tissue,sep = "/"),full.names = TRUE) %>% 
    map(myfuns) %>% purrr::reduce(inner_join, by = "ID")
  
  edger.zt <- list.files(path = paste("/data/msun/projects/06_GSEA/edgeR/kegg/table_zt4vs16",tissue,sep = "/"), full.names = TRUE) %>% 
    map(myfuns) %>% purrr::reduce(inner_join, by = "ID")
  
  deseq.sedrw <- list.files(path = paste("/data/msun/projects/06_GSEA/DESeq2/kegg/table_sedvsrw",tissue,sep = "/"), full.names = TRUE) %>% 
    map(funs) %>% purrr::reduce(inner_join, by = "ID")
  
  deseq.zt <- list.files(path = paste("/data/msun/projects/06_GSEA/DESeq2/kegg/table_zt4vs16",tissue,sep = "/"), full.names = TRUE) %>% 
    map(funs) %>% purrr::reduce(inner_join, by = "ID")

    #kegg info
  kegg <- read.csv("/data/msun/projects/06_GSEA/DESeq2/kegg/table_sedvsrw/BF/BFF_16_SedvsF_16_RW.csv",row.names = 1) %>% dplyr::select(1:3)
  #last merge
  data.list <- list(kegg,edger.sedrw,deseq.sedrw,edger.zt,deseq.zt)
  combined.table <- data.list %>% purrr::reduce(inner_join, by = "ID") 
  print(tissue)
  openxlsx::write.xlsx(combined.table,file =
              paste(paste("/data/msun/projects/06_GSEA/combined_kegg",tissue,sep = "/"),".xlsx",sep = ""))
}
