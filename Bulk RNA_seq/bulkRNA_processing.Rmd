...
library(tidyverse)
files <- dir(path = "/data/msun/projects/pandas/geraldine/raw_data/readcountstab", pattern = "*.tab", full.names = T) #path of all tab data
count <- files %>%
...
datasets <- files %>%
  stringr::str_replace("/data/msun/projects/pandas/geraldine/raw_data/readcountstab/", "") %>% # replace pathname at the beginning of filename
...
library(edgeR)
#correct_name=read.csv("/data/msun/projects/pandas/geraldine/raw_data/correct_samplenames.csv",row.names = 1) 
...
#add meatadata
metadata <- read.csv("/data/msun/projects/pandas/geraldine/raw_data/samplenames.csv") 
...
write.csv(metadata,file = "/data/msun/projects/pandas/geraldine/raw_data/samples_metadata_correctnames.csv")

#updata metadata by adding library size
summary <- dge$samples %>% .[,-which(colnames(.) %in% c("correctednames","group","norm.factors"))]
write.csv(summary,file = "/data/msun/projects/pandas/geraldine/01_preprocess/summary_sample.csv")
```
...
save(count,count_filter,count_filter_sample_filter,dge,dge.list,dge.norm,expr,expr_symbol,expr.list,file = "/data/msun/projects/pandas/geraldine/01_preprocess/preprocess_filtered.Rdata")

write.csv(dge$counts,file = "/data/msun/projects/pandas/geraldine/01_preprocess/filtered_counts.csv")
write.csv(expr,file = "/data/msun/projects/pandas/geraldine/01_preprocess/filtered_norm.csv")
...
write.csv(norm.list,file = "/data/msun/projects/pandas/geraldine/01_preprocess/norm_deseq.csv")
...
#add metadata
metadata <- read.csv("/data/msun/projects/pandas/geraldine/raw_data/metadata.csv") 
...
#write.table(nofilter_expr_symbol,file='/data/msun/projects/pandas/geraldine/04_compass/nofilter_expr_symbol.tsv',sep = "\t",quote = FALSE)
#write.table(expr_symbol,file='/data/msun/projects/pandas/geraldine/04_compass/expr_symbol.tsv',sep = "\t",quote = FALSE)
#modify in spreadsheet
save(nofilter_dge.norm,nofilter_expr,nofilter_expr_symbol,file = "/data/msun/projects/pandas/geraldine/01_preprocess/preprocess_nofilter.Rdata")
#```
...
gtf <- as.data.frame(rtracklayer::import("/data/msun/projects/pandas/geraldine/raw_data/Mus_musculus.GRCm38.102.gtf"))
...
write.csv(geneid_efflen,"/data/msun/projects/pandas/geraldine/01_preprocess/mus_genelength_id.csv")
...
write.table(tpm_symbol_duplicated,file='/data/msun/projects/pandas/geraldine/04_compass/tpm_symbol_duplicated.tsv',sep = "\t",row.names = FALSE)
save(tpm_geneid,tpm_symbol,tpm_symbol_duplicated,file = "/data/msun/projects/pandas/geraldine/01_preprocess/tpm.Rdata")
```
...
import numpy as np
import pandas as pd
df = pd.read_csv('/data/msun/projects/pandas/geraldine/04_compass/tpm_symbol_duplicated.tsv', sep='\t', index_col=None, header=0)
df.dropna(subset=['gene_name'], inplace=True)
df=df.rename(columns = {'gene_name':'Gene Symbol'})
df.to_csv('/data/msun/projects/pandas/geraldine/04_compass/tpm_symbol.tsv', sep='\t', index=False)
df
```
...
write.csv(df,"/data/msun/projects/pandas/geraldine/01_preprocess/qc_by_featureRNA.csv")
...
write.csv(df,"/data/msun/projects/pandas/geraldine/01_preprocess/allgene_biotype_num_percentage.csv")
...
write.csv(df,"/data/msun/projects/pandas/geraldine/01_preprocess/ubiquitousgene_biotype_num_percentage.csv")
